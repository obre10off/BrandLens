...
import { Edit2, Trash2, TrendingUp, TrendingDown, BarChart3 } from 'lucide-react';
...
export default async function CompetitorsPage() {
  ...
  // Check plan limits
  const competitorLimit = currentOrg.trial?.status === 'active' 
    ? 3 
  ...
  const canAddMore = competitorLimit === -1 || competitorsWithStats.length < competitorLimit;
          ...
          <Card className="md:col-span-2 lg:col-span-3">
            <CardContent className="text-center py-12">
              ...
              <p className="text-sm text-black">
                Add competitors to track how your brand compares in AI conversations
              </p>
            </CardContent>
          </Card>
        ) : (
          competitorsWithStats.map(({ competitor, mentionCount, positiveMentions, negativeMentions }) => {
              ...
              <Card key={competitor.id} className="card-hover">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-black">{competitor.name}</CardTitle>
                      {competitor.domain && (
                        <CardDescription className="text-black">
                          <a 
                ...
                <CardContent>
                  <div className="space-y-4">
                    {competitor.description && (
                      <p className="text-sm text-black">{competitor.description}</p>
                    )}
                    
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm font-medium text-black">Total Mentions</p>
                        <p className="text-2xl font-bold text-black">{mentionCount}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-black">Sentiment</p>
                        <div className="flex items-center gap-2">
                          <p className="text-2xl font-bold text-black">{sentimentScore}%</p>
                          {mentionCount > 0 && (
                            sentimentScore >= 70 ? (
