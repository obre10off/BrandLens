...
export const accounts = pgTable(
  ...
  table => ({
    ...
    providerUserIdx: unique('account_provider_user_idx').on(
      table.providerId,
      table.userId
    ),
  })
...
export const organizationMembers = pgTable(
  ...
  table => ({
    uniqueMember: unique('unique_org_member').on(
...
// Query templates
export const queryTemplates = pgTable(
  'query_templates',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const projectQueries = pgTable(
  'project_queries',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    projectId: uuid('project_id')
...
export const brandMentions = pgTable(
  'brand_mentions',
  {
    ...
    projectId: uuid('project_id')
      .references(() => projects.id)
      .notNull(),
    queryExecutionId: uuid('query_execution_id').references(
      () => queryExecutions.id
    ),
...
export const projectQueriesRelations = relations(
  projectQueries,
  ({ one, many }) => ({
    project: one(projects, {
