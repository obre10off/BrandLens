...
export function BrandHealthScore({ data, className }: BrandHealthScoreProps) {
  const { overallScore, components, totalMentions, trend, previousScore } =
    data;
  ...
  return (
    <Card
      className={cn(
        'brand-health-card bg-gray-50/30 shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-200/40',
        className
      )}
    >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-gray-600 text-base">
              Overall AI visibility and perception across {totalMentions}{' '}
              mentions
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="p-4 rounded-lg bg-blue-50 border border-blue-100">
          ...
          <div className="space-y-1 text-xs text-blue-800">
            {overallScore >= 80 && (
              <p>
                • Your brand has excellent AI visibility and positive sentiment
              </p>
            )}
            {overallScore < 80 && components.visibility < 60 && (
              <p>
                • Focus on increasing brand visibility through content
                optimization
              </p>
            )}
            {components.sentiment < 60 && (
              <p>
                • Consider addressing sentiment issues through better
                positioning
              </p>
            )}
            {components.competitiveness < 50 && (
              <p>
                • Competitors are outperforming you in AI mentions - analyze
                their strategy
              </p>
            )}
            {trend === 'up' && (
              <p>
                • Your brand perception is trending upward - maintain current
                strategy
              </p>
            )}
            {trend === 'down' && (
              <p>
                • Brand perception declining - review content and competitor
                activity
              </p>
            )}
          </div>
